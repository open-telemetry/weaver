...
function SpanDetail() {
  ...
  const { data, error } = useResourceFetch<SpanResponse>(type, getSpan)
  const { copied, copyToClipboard } = useCopyToClipboard()

  function formatRequirementLevel(requirement_level: SpanAttribute['requirement_level']): { label: string; badgeClass: string } {
  ...
  function formatType(type: SpanAttribute['type']): string {
      ...
      {error ? (
        ...
        <>
          ...
          {data.deprecated && typeof data.deprecated === 'object' && (
            <div className="alert alert-warning" role="alert">
              ...
              <div>
                ...
                <div className="text-sm">{data.deprecated.note || 'This span is deprecated.'}</div>
                {data.deprecated.renamed_to && (
                  <div className="text-sm mt-1">
                    Use <Link to="/span/$type" params={{ type: data.deprecated.renamed_to }} className="link">{data.deprecated.renamed_to}</Link> instead.
                  </div>
                )}
              </div>
            </div>
          ...
          {data.attributes && data.attributes.length > 0 && (
            <div className="card bg-base-200">
              <div className="card-body">
                <h2 className="card-title">Span Attributes</h2>
                <div className="overflow-x-auto">
                  <table className="table">
                    <caption className="sr-only">Span attributes with type and requirement</caption>
                    <thead>
                      <tr>
                        <th>Attribute</th>
                        <th>Type</th>
                          ...
                          <tr key={index}>
                            ...
                            <td>
                              {attr.sampling_relevant && (
                                <span className="badge badge-accent">sampling</span>
                              )}
                            </td>
