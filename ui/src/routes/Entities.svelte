<script>
  ...
  async function loadData() {
    try {
      data = await getEntities({ stability: stability || undefined, limit, offset });
      updateURL();
    } catch (e) {
      error = e.message;
    }
      ...
      <table class="table table-zebra">
        ...
        <tbody>
          {#each data.items as entity}
            <tr class="hover" class:opacity-50={entity.deprecated}>
              <td>
                <a href="/entities/{entity.type}" use:link class="link link-primary font-mono text-sm">
                  {entity.type}
                </a>
              ...
              <td class="max-w-md truncate"><InlineMarkdown content={entity.brief || '-'} /></td>
              <td><StabilityBadge stability={entity.stability} /></td>
            </tr>
          {/each}
        </tbody>
      </table>
