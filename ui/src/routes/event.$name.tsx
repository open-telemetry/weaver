...
function formatType(type: EventAttribute['type']): string {
  return typeof type === 'string' ? type : 'enum'
}

function EventDetail() {
  const { name } = Route.useParams()
  const { data, error } = useResourceFetch<EventResponse>(name, getEvent)
      ...
      {error ? (
        ...
        <>
          ...
          {data.deprecated && typeof data.deprecated === 'object' && (
            <div className="alert alert-warning" role="alert">
              ...
              <div>
                ...
                <div className="text-sm">{data.deprecated.note || 'This event is deprecated.'}</div>
                {data.deprecated.renamed_to && (
                  <div className="text-sm mt-1">
                    Use <Link to="/event/$name" params={{ name: data.deprecated.renamed_to }} className="link">{data.deprecated.renamed_to}</Link> instead.
                  </div>
                )}
              </div>
            </div>
          ...
          {data.attributes && data.attributes.length > 0 && (
            <div className="card bg-base-200">
              <div className="card-body">
                <h2 className="card-title">Event Attributes</h2>
                <div className="overflow-x-auto">
                  <table className="table">
                    <caption className="sr-only">Event attributes with type and requirement</caption>
                    <thead>
                      <tr>
                        <th>Attribute</th>
                        <th>Type</th>
