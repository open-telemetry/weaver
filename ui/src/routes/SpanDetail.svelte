<script>
  ...
  import { getSpan } from '../lib/api.js';
...
<div class="space-y-4">
  ...
  {:else}
    <div class="flex items-center gap-4 flex-wrap">
      ...
      <span class="badge badge-outline">Span</span>
      <span class="badge badge-lg badge-secondary">{data.kind || 'internal'}</span>
    ...
    {#if data.deprecated}
      <div class="alert alert-warning">
        ...
        <div>
          ...
          <div class="text-sm">{data.deprecated.note || 'This span is deprecated.'}</div>
          {#if data.deprecated.renamed_to}
            <div class="text-sm mt-1">Use <a href="/span/{data.deprecated.renamed_to}" use:link class="link">{data.deprecated.renamed_to}</a> instead.</div>
          {/if}
        </div>
      </div>
    {/if}
    ...
    {#if data.attributes?.length}
      <div class="card bg-base-200">
        <div class="card-body">
          <h2 class="card-title">Span Attributes</h2>
              ...
              <tbody>
                {#each data.attributes as attr}
                  <tr>
                    ...
                    <td>
                      {#if attr.sampling_relevant}
                        <span class="badge badge-accent">sampling</span>
                      {/if}
                    </td>
