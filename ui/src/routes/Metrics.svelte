<script>
  ...
  async function loadData() {
    try {
      data = await getMetrics({ stability: stability || undefined, limit, offset });
      updateURL();
    } catch (e) {
      error = e.message;
    }
...
<div class="space-y-4">
  ...
  {:else}
    <p class="text-sm text-base-content/70">
      Showing {data.count} of {data.total} metrics
    </p>
    ...
    <div class="overflow-x-auto">
      <table class="table table-zebra">
        ...
        <tbody>
          {#each data.items as metric}
            <tr class="hover" class:opacity-50={metric.deprecated}>
              <td>
                <a href="/metrics/{metric.name}" use:link class="link link-primary font-mono text-sm">
                  {metric.name}
                </a>
              ...
              <td class="badge badge-outline">{metric.instrument || '-'}</td>
              <td class="font-mono text-sm">{metric.unit || '-'}</td>
              <td class="max-w-md truncate"><InlineMarkdown content={metric.brief || '-'} /></td>
              <td><StabilityBadge stability={metric.stability} /></td>
            </tr>
          {/each}
        </tbody>
      </table>
