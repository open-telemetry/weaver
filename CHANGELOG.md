...
- Fix `dependencies` in the materialized (forge) schema listing every transitive dependency as a direct one, with its subtree materialized again underneath. It now follows the manifests, so each registry appears under the one that depends on it. ([#1730](https://github.com/open-telemetry/weaver/pull/1730) by @jerbly)
