...
- 💥 BREAKING CHANGE 💥 OTLP receiver: (Fixes: [#1626](https://github.com/open-telemetry/weaver/issues/1626)) decouple retrieving the `live-check --output http` report from stopping the process. `GET /live-check/report` now finalizes (if needed) and serves the report — callable any number of times, with no timeout on how long a client takes to read it. `POST /stop` no longer returns a report; it purely terminates the process. This removes the underlying race that could cut a `/stop` response short, and also unifies graceful shutdown of the gRPC and HTTP admin servers across every stop signal (CTRL+C, SIGHUP, `/stop`, inactivity) so none of them can cut an in-flight request short. `--inactivity-timeout` now waits for an explicit stop instead of exiting immediately when `--output http` is set, so a client always has a chance to retrieve the report. (by @jerbly)
