...
- Add matchers to live-check, so a span or a log with no identifier of its own can be compared with a signal in a v2 registry. A `[[live-check.matchers]]` entry names a `sample_type`, an optional CEL `when` expression that detects a signature in the sample, and the `signal` or `attribute_groups` to compare it with. Expressions are compiled and checked against the registry at startup, so a typo fails the run rather than the stream. A matched span is then checked against that span signal, the way metrics and events already are: its attributes against the signal's requirement levels, and its kind against the signal's kind. Three new findings: `unmatched_sample` when an untyped sample matched no matcher, `matcher_conflict` when more than one matcher that applied set a signal, and `kind_mismatch` when a matched span's kind differs from the signal's. Matchers need a v2 registry. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
