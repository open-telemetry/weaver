...
- 💥 BREAKING CHANGE 💥 OTLP receiver: (Fixes: [#1626](https://github.com/open-telemetry/weaver/issues/1626)) decouple retrieving the `live-check --output http` report (now served from `GET /live-check/report`, callable any number of times) from stopping the process (`POST /stop`, which no longer returns a report), and unify graceful shutdown of the gRPC and HTTP admin servers across every stop signal so none of them can cut an in-flight request short. (by @jerbly)
