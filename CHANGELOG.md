...
- 💥 BREAKING CHANGE 💥 Fix live-check treating any key that starts with a template's name as an instance of it: `http.request.headers.host` no longer matches the template `http.request.header`, and now raises `missing_attribute`. The namespace separator must follow the template name. `--namespace-separator` (default `.`) is now a global flag on every subcommand, so a `namespace_separator` key under `[mcp]` in `.weaver.toml` must move to the root of the file. ([#1743](https://github.com/open-telemetry/weaver/issues/1743) by @jerbly)
