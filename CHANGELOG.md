...
- Add `-D`/`--param` and `--params` to `weaver registry live-check`, which pass parameters to the output template as the `params` global, as `registry generate` already did. The ansi format reads `show_finding_id`: set it to label a finding with its id instead of its level, keeping the colour the level gives it. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
- Fix the ansi live-check output running an instrumentation scope's findings onto the scope header line, and losing the blank line that separates a scope from what follows. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
- `unexpected_attribute` is now raised for an attribute that only a dependency declares, on any matched sample, not only on one whose match names a signal or attribute groups. Reaching the base definitions at all means nothing in this registry declared the attribute. Requires `search_all_attributes`. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
- Live-check now checks a span's `entity_associations` against the resource, as it already did for metrics and logs. A v2 span reaches this only through a matcher, which is what gives the span its signal. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
- Fix a matcher's `signal` not reaching the checks for a metric or a log. The match was made after the advisors had run, so a matcher naming a metric signal did not drive `unit_mismatch`, `unexpected_instrument` or the attribute requirement levels, and only the span path was correct. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
- `[[live-check.matchers]]` now applies to every sample type it accepts. A matcher on `span_event`, `span_link`, `resource`, `instrumentation_scope` or `profile` compiled and validated but never fired. A span event and a span link now match on their own instead of inheriting the span's match, so their attributes are checked against the signal and attribute groups their own matchers name. `unmatched_sample` is raised only for a sample type some matcher targets. ([#1721](https://github.com/open-telemetry/weaver/pull/1721) by @jerbly)
