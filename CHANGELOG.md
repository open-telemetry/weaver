...
- Fix the materialized (forge) schema presenting every transitive dependency as a direct one. It was built from the resolved schema's `dependencies`, which lists every registry the schema was built from, direct and transitive alike. A registry two hops away therefore appeared next to the one that actually depends on it, with its subtree materialized again underneath. The tree now follows the manifests. A registry consumed as an already-resolved artifact keeps its shape too, read from its publication manifest. Where no manifest is available to say what a registry declared, the dependencies are listed flat rather than nested. ([#TBD](https://github.com/open-telemetry/weaver/pull/TBD) by @jerbly)
