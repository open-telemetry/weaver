...
- Fix findings on resource samples being emitted without the resource's attributes. `Sample::resource()` returned `None` for a resource sample: it only returned a sample's parent resource, and a resource has no parent. A finding on a resource-level attribute therefore reached `--emit-otlp-logs` with none of its source resource's attributes, while findings on span, metric and log samples carried them. ([#1612](https://github.com/open-telemetry/weaver/pull/1612) by @fabiovincenzi)
