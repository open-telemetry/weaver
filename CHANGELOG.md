...
- 💥 BREAKING CHANGE 💥 `registry infer` renames `--grpc-address` and `--grpc-port` to `--otlp-grpc-address` and `--otlp-grpc-port` to match `registry live-check`. Its listener settings move from `[infer]` to `[infer.otlp]` (`grpc_address`, `grpc_port`, `admin_port`, `inactivity_timeout`), the same shape as `[live-check.otlp]`, and an unknown key under `[infer]` now stops the run. ([#1741](https://github.com/open-telemetry/weaver/issues/1741) by @jerbly)
