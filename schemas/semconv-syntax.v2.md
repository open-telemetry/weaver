...
- `name` - Optional. Overrides the [span name](#span-name) specification from the referenced base span. If set, the refinement's `name.note` is used; otherwise, the base span's `name.note` is inherited.
