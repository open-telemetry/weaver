...
The `name` of an item in the semantic conventions is a persistant ID that should not change
across versions. However, there are cases where renaming an item is necessary. In such cases,
the procedure for renaming is to deprecate the old item and introduce a new item with the new
name. In the `deprecated` field of the old item, the new item is referenced as the replacement
with additional information about the type of renaming.

Multiple forms of renaming are supported in the semantic conventions.

- Basic renaming: A single item is renamed to a new name. Backward and forward transformations
  are straightforward.
- Conditional renaming: An item is renamed to a new name based on a condition. The condition
  can be based on the value of another item or a combination of items.
- Split: An item is split into multiple items. Backward and forward transformations must be
  defined explicitly.
- Merge: Multiple items are merged into a single item. Backward and forward transformations
  must be defined explicitly.

The next sections describe each form of renaming in more detail.

Note: The name of a span is not yet properly defined in the semantic conventions.
