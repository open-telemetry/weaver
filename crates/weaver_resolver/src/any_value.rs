...
pub fn resolve_any_value_spec(value: &AnyValueSpec) -> AnyValue {
    match value {
        AnyValueSpec::Map { fields, .. } => {
            ...
            construct_any_value_common(value, Some(resolved_fields), None)
        }
        AnyValueSpec::Enum { members, .. } => {
            construct_any_value_common(value, None, Some(members.to_vec()))
        }
        _ => construct_any_value_common(value, None, None),
    }
}
