...
# Event with an event_name that we can compare against the matched event in the model.
...
deny contains make_advice(advice_type, advice_level, advice_context, message) if {
    ...
    not contains(input.sample.event.body, phrase)
    advice_type := "required_phrase_missing"
    advice_level := "violation"
    advice_context := {
        "event_name": input.sample.event.event_name,
        "required_phrase": phrase
    }
    message := sprintf("Event '%s' body does not contain the required phrase: '%s'", [input.sample.event.event_name, phrase])
}

# Event with an empty event.name that we can still have policies against.
# Must not have an empty body.
deny contains make_advice(advice_type, advice_level, advice_context, message) if {
    input.sample.event
    input.sample.event.event_name == ""
    input.sample.event.body == ""
