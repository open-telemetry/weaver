...
| ----------------------------------------------------- | ------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Metric                                                | Metric             | Metric name            | Look up the name in the registry. If there is nothing we get a `missing_metric` finding.                                                                                                                                                                             |
...
| Log                                                   | Event              | `event_name`           | Look up the event name in the registry. A log with an empty `event_name` is not matched at all and we get no finding for it, so its attributes are only compared against the registry as a whole.                                                                   |
...
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `attributes["key"]`                                                                               | Any sample                                      | The attributes on the sample, as a map. On a metric this is all of the data point attributes together.      |
...
| `event_name`, `severity_text`, `severity_number`, `body`                                          | Log                                             | The fields on the log record.                                                                               |

CEL brings the operators you would expect, `==`, `!=`, `&&`, `||`, `!` and brackets, along with the string methods `matches`, `startsWith`, `endsWith` and `contains`. Everything in a matcher is plain CEL, so anything you already know about the language holds here.
...
We compile and lint every expression at startup, so a matcher that does not parse, or that has a problem such as an unguarded read, a test on the wrong key, or a variable we do not have for that sample type, stops us there. The lint does not follow branches, so an expression can still error when it runs. We log that once as a diagnostic against the matcher, not as a finding.
...
| `unexpected_attribute` | Improvement | A matched sample has an attribute that is not in the comparison set.                                                                                            |
