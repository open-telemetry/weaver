...
impl Logger {
    ...
    pub fn warn_count(&self) -> usize {
        self.messages.lock().expect("Failed to lock messages")
            .iter()
    ...
    pub fn messages(&self) -> Vec<LogMessage> {
        self.messages.lock().expect("Failed to lock messages").clone()
    }
}
...
impl crate::Logger for Logger {
    ...
    fn add_style(&self, _name: &str, _styles: Vec<&'static str>) -> &Self { self }
