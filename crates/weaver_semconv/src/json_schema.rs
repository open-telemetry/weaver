...
impl JsonSchemaValidator {
    ...
    pub fn validate_yaml(
        ...
        for error in self.validator.iter_errors(&json) {
            let src = serde_json::to_string_pretty(error.instance.as_ref()).unwrap_or_else(|_| "NA".to_owned());
...
pub struct YamlDocument<'a> {
    content: &'a str,
    yaml: Yaml<'a>,  // This likely borrows from content via 'a
    lines: Vec<usize>, // byte offsets for each line start
}

impl<'a> YamlDocument<'a> {
    /// Create a new YamlDocument from a file path
    pub fn from_str(content: &'a str) -> Result<Self, Box<dyn std::error::Error>> {
        let docs = Yaml::load_from_str(content)?;
        let yaml = docs.into_iter().next()
            .ok_or("No YAML document found")?;
        ...
        Ok(YamlDocument { content, yaml, lines })
    }

    /// Find the position of a YAML block using simple XPath syntax
    pub fn find_block_position(&self, xpath: &str) -> Result<BlockPosition, Box<dyn std::error::Error>> {
    ...
    fn navigate_to_block<'b>(&self, yaml: &Yaml<'b>, path: &[PathComponent]) -> Result<Yaml<'b>, Box<dyn std::error::Error>> {
        ...
        for component in path {
            match component {
                ...
                PathComponent::Index(index) => {
                    ...
                    if current.is_badvalue() {
                        return Err(format!("Index {} out of bounds or not a sequence", index).into());
                    }
                }
            }
        }
    ...
    fn find_block_boundaries(&self, path: &[PathComponent]) -> Result<BlockPosition, Box<dyn std::error::Error>> {
        if path.is_empty() {
        ...
        for (component_idx, component) in path.iter().enumerate() {
            match component {
                ...
                PathComponent::Index(target_index) => {
                    ...
                    if !found {
                        return Err(format!("Could not find array index {} in source", target_index).into());
                    }
                }
            }
        }
    ...
    fn byte_to_position(&self, byte_offset: usize) -> Position {
        for (line_num, &line_start) in self.lines.iter().enumerate() {
            if line_start > byte_offset {
                let prev_line_start = if line_num > 0 { self.lines[line_num - 1] } else { 0 };
                return Position {
