...
/// The variables an expression reads from its environment.
///
/// A variable that a comprehension binds, such as `k` in `exists(k, ...)`,
/// is not included. This set is what [`Referenced`](crate::Referenced)
/// holds. A `Bindings` implementation uses it to build only the variables
/// the expression reads. A `Scope` uses the union of several expressions to
/// bind once for all of them.
///
/// `Program::references().variables()` from the `cel` crate includes the
/// loop variables, so it cannot be used here.
...
fn collect(expression: &IdedExpr, bound: &mut Vec<String>, free: &mut BTreeSet<String>) {
    match &expression.expr {
        // A name that starts with `@` is generated by the parser. It is not a
