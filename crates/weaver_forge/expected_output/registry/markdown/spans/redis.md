...
| `db.operation` | `string` | Conditionally Required - If `db.statement` is not applicable. | The name of the operation being executed, e.g. the [MongoDB command name](https://docs.mongodb.com/manual/reference/command/#database-operations) such as `findAndModify`, or the SQL keyword.
 |
| `db.redis.database_index` | `int` | Conditionally Required - If other than the default database (`0`). | The index of the database being accessed as used in the [`SELECT` command](https://redis.io/commands/select), provided as an integer. To be used instead of the generic `db.name` attribute.
 |
