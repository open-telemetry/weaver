...
| `db.system` | Enum | Required | An identifier for the database management system (DBMS) product being used. See below for a list of well-known identifiers.
 |
| `db.name` | `string` | Conditionally Required - If applicable. | The keyspace name in Cassandra.
 |
...
| `server.port` | `int` | Conditionally Required - If using a port other than the default port for this DBMS and if `server.address` is set. | Server port number.
 |
| `db.cassandra.consistency_level` | Enum | Recommended | The consistency level of the query. Based on consistency values from [CQL](https://docs.datastax.com/en/cassandra-oss/3.0/cassandra/dml/dmlConfigConsistency.html).
 |
| `db.cassandra.coordinator.dc` | `string` | Recommended | The data center of the coordinating node for a query.
 |
| `db.cassandra.coordinator.id` | `string` | Recommended | The ID of the coordinating node for a query.
 |
| `db.cassandra.idempotence` | `boolean` | Recommended | Whether or not the query is idempotent.
 |
| `db.cassandra.page_size` | `int` | Recommended | The fetch size used for paging, i.e. how many rows will be returned at once.
 |
| `db.cassandra.speculative_execution_count` | `int` | Recommended | The number of times a query was speculatively executed. Not set or `0` if the query was not executed speculatively.
 |
| `db.cassandra.table` | `string` | Recommended | The name of the primary Cassandra table that the operation is acting upon, including the keyspace name (if applicable).
 |
